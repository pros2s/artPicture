...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_sliders__WEBPACK_IMPORTED_MODULE_1__["default"])('.main-slider-item', '.main-prev-btn', '.main-next-btn', true);
  ...
  Object(_modules_dragDrop__WEBPACK_IMPORTED_MODULE_12__["default"])('.file_upload');
});
...
var dragDrop = function dragDrop(dragSelector) {
  var elemsToDrop = document.querySelectorAll(dragSelector); //dragenter - элемент в пределах dropArea
  ...
  elemsToDrop.forEach(function (elem) {
    elem.addEventListener('drop', function (e) {
      elem.files = e.dataTransfer.files; //cut the name of file(s)

      var dots;
      var arr = elem.files[0].name.split('.');
...
var filter = function filter(trigger, product) {
  ...
  picGroup.addEventListener('click', function (e) {
    ...
    var showPicture = function showPicture(name) {
      picture.forEach(function (item) {
...
var forms = function forms() {
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      item.parentNode.appendChild(statusMessage); //Hide form

      item.classList.add('animated', 'fadeOutUp');
      setTimeout(function () {
        item.style.display = 'none';
      }, 400); //Spinner for loading message

      var statusImg = document.createElement('img');
      statusImg.setAttribute('src', message.spinner);
      statusImg.classList.add('animated', 'fadeInUp');
      statusMessage.appendChild(statusImg); //Text for loading message

      var textMessage = document.createElement('div');
      textMessage.textContent = message.loading;
      statusMessage.appendChild(textMessage); //class FormData to create template of data

      var formData = new FormData(item); //Write right path to api if .popup-design exist at any parents of not
...
var loadStyles = function loadStyles(trigger, wrapper) {
  var btn = document.querySelector(trigger);

  var createCards = function createCards(res) {
...
var modals = function modals() {
  var btnChecked = false; //Popup modal by click on smth
  ...
  var bindmodal = function bindmodal(triggerSelector, modalSelector, closSelector) {
    ...
    trigger.forEach(function (item) {
      item.addEventListener('click', function (e) {
        ...
        btnChecked = true; //Destroing trigger for fixed gift
  ...
  var openByScrolldown = function openByScrolldown(selector) {
    window.addEventListener('scroll', function () {
      //This var for old browsers(documentElement for modern browsers but body for outdated)
      var scrollToBottom = Math.max(document.documentElement.scrollHeight, document.body.scrollHeight);
...
var showSizesPics = function showSizesPics(picSize) {
  ...
  pictureSize.forEach(function (block) {
...
var sliders = function sliders(slides, prev, next) {
  var updown = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
  ...
  var changeClasses = function changeClasses(firstClass, secondClass) {
  ...
  try {
    //To prevent the code from stopping
    var prevBtn = document.querySelector(prev),
        nextBtn = document.querySelector(next); //Functional for prev button
